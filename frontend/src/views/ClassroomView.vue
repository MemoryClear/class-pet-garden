...
// 声调数据（4声）
const toneList = ref([
  { tone: 1, icon: '🔵', mark: 'ā', name: '高平', desc: '声音高而平稳，不升不降', example: '妈 / 诗 / 天 / 哥' },
  { tone: 2, icon: '🟡', mark: 'á', name: '高升', desc: '声音从中高升到高', example: '麻 / 骑 / 来 / 拔' },
  { tone: 3, icon: '🟠', mark: 'ǎ', name: '降升', desc: '先降再升，曲调先低后高', example: '马 / 我 / 走 / 有' },
  { tone: 4, icon: '🔴', mark: 'à', name: '高降', desc: '声音从高快速降到低', example: '骂 / 气 / 去 / 爸' }
])
...
// 声母/韵母发声映射
// 规则：卡片 sound（无调号）→直接查表；带调号词例（如 mā）→最长前缀匹配
const pinyinCharMap = {
  // 声母（无调号，供 item.sound 直接查找）
  'b':'波','p':'坡','m':'摸','f':'佛',
  'd':'得','t':'特','n':'讷','l':'乐',
  'g':'哥','k':'科','h':'喝',
  'j':'基','q':'欺','x':'希',
  'zh':'知','ch':'蚩','sh':'诗','r':'日',
  'z':'资','c':'雌','s':'思','y':'衣','w':'乌',
  // 韵母（无调号，韵母卡片 sound 字段）
  'a':'啊','o':'哦','e':'婀','i':'衣','u':'乌','v':'淤',
  'ai':'哀','ei':'诶','ui':'威','ao':'凹','ou':'欧','iu':'忧','ie':'椰','üe':'约','er':'耳',
  'an':'安','en':'恩','in':'音','un':'温','ün':'晕',
  'ang':'昂','eng':'鞥','ing':'英','ong':'翁',
  // 带调号完整音节（pinyinToChinese 最长前缀匹配兜底，防止多音节词被错误拆分）
  'mā':'妈','má':'麻','mǎ':'马','mà':'骂',
  'shī':'诗','shí':'时','shǐ':'使','shì':'是',
  'tiān':'天','tián':'田','tiǎn':'舔','tiàn':'掭',
  'gē':'哥','gě':'个','gè':'各',
  'qí':'骑','qǐ':'起','qì':'气',
  'lái':'来','lǎi':'睐',
  'bá':'拔',
  ...
  'ūn':'温','ún':'文','ǔn':'稳','ùn':'问',
  'ǖn':'晕','ǘn':'云','ǚn':'允','ǜn':'运',
  'āng':'昂','áng':'昂','ǎng':'航','àng':'盎',
  'ēng':'鞥','éng':'鞥','ěng':'鞥','èng':'鞥',
  'īng':'英','íng':'赢','ǐng':'影','ìng':'映',
  'ōng':'翁','óng':'同','ǒng':'筒','òng':'用',
  // 声母（带调号，用于 example 中含声母的解析兜底）
  'bō':'波','bó':'博','bǒ':'跛','bò':'薄',
  'pō':'坡','pó':'婆','pǒ':'叵','pò':'破',
  'mō':'摸','mó':'魔','mǒ':'抹','mò':'莫',
  'fō':'佛','fó':'佛',
  'dē':'得','dé':'德','děi':'得','dè':'得',
  'tē':'特','té':'特','tè':'特',
  'nē':'讷','né':'哪','něi':'馁','nè':'讷',
  'lē':'乐','lé':'乐','lě':'仂','lè':'乐',
  'gē':'哥','gé':'革','gě':'个','gè':'各',
  'kē':'科','ké':'咳','kě':'可','kè':'克',
  'hē':'喝','hé':'合','hě':'何','hè':'鹤',
  ...
  'qī':'七','qí':'其','qǐ':'起','qì':'气',
  'xī':'西','xí':'习','xǐ':'洗','xì':'细',
  'zhī':'知','zhí':'直','zhǐ':'只','zhì':'至',
  'chī':'吃','chí':'持','chǐ':'尺','chì':'赤',
  'shī':'师','shí':'时','shǐ':'使','shì':'是',
  'rì':'日',
  'zī':'资','zí':'自','zǐ':'子','zì':'字',
  'cī':'雌','cí':'词','cǐ':'此','cì':'次',
  'sī':'思','sī':'思','sǐ':'死','sì':'四',
...
// 将拼音字符串（含声母+韵母）转换为中文读音
// 策略：优先最长前缀匹配（完整音节 > 声母+韵母）
function pinyinToChinese(pinyin) {
  // 先尝试直接命中（覆盖带调号完整音节）
  if (pinyinCharMap[pinyin]) return pinyinCharMap[pinyin]
  // 从前向后贪心匹配（覆盖声母+韵母的拆分情况）
  let result = ''
  let i = 0
  while (i < pinyin.length) {
    let matched = pinyin.charAt(i)
    // 尝试二字母/三字母前缀（处理 zh/ch/sh 和带调韵母）
    if (i + 2 <= pinyin.length) {
      const two = pinyin.substring(i, i + 2)
      if (pinyinCharMap[two]) {
        matched = pinyinCharMap[two]
        i += 2
      } else if (i + 3 <= pinyin.length) {
        const three = pinyin.substring(i, i + 3)
        if (pinyinCharMap[three]) {
          matched = pinyinCharMap[three]
          i += 3
        } else {
          i++
        }
...
async function speakPinyin(item) {
  // 韵母卡片 sound 为无调号拼音（如 ai/ang/eng），直接查表
